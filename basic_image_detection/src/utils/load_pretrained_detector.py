...
def load_backbone(model_type="ssd_mobilenet_v2", input_shape=(640, 640, 3)):
    ...
    Args:
        model_type: One of "ssd_mobilenet_v2", "mobilenet_v2", "resnet50", "efficientnet_b0"
        input_shape: Input image shape (height, width, channels)
    
    Returns:
        Keras model (backbone only, no detection head)
    """
    print(f"[Model] Loading {model_type} backbone with ImageNet weights...")
    
    if model_type == "ssd_mobilenet_v2" or model_type == "mobilenet_v2":
        ...
        raise ValueError(
            ...
            f"Available: ssd_mobilenet_v2, mobilenet_v2, resnet50, efficientnet_b0"
        )
